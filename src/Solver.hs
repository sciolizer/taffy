...
backtrack = do
  ...
  case mbVar of
    ...
    Just var -> do
      ...
      -- The argument to runNew should be (return True), since all
      -- learnt clauses can be garbage collected.
      liftIO $ unset var
...
propagate vs p clauses = do
  (newConstraints, implications) <- foldThing genImplications clauses
